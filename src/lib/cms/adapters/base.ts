...
export abstract class BaseCMSAdapter implements CMSAdapter {
  ...
  /**
     * Make an API request that requires an active connection.
  ...
  protected async apiRequest<T>(
        ...
        return this.rawApiRequest<T>(path, options);
  }

  /**
     * Make a raw API request without checking connection status.
     * Used during the connect() flow before connected=true.
     */
