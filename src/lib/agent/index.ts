...
export class PublishingAgent {
  ...
  private loadPlaybook(playbookPath: string): string {
        try {
                ...
                return `You are an expert SEO manager. Follow best practices for meta titles (max 60 chars), meta descriptions (150-160 chars), slugs (short, keyword-rich, hyphenated), Open Graph tags, and content quality. Always identify and flag missing required fields.`;
        }
  }
  ...
  private buildSystemPrompt(cmsSchema: CMSSchema | null): string {
      ...
      if (cmsSchema) {
              const categories = cmsSchema.categories || [];
              const tags = cmsSchema.tags || [];
              const customFields = cmsSchema.custom_fields || [];
              prompt += `\n\nTarget CMS Schema:
              Content Types: ${JSON.stringify(cmsSchema.content_types || [], null, 2)}
              Available Categories: ${JSON.stringify(categories.slice(0, 50))}
              Available Tags: ${JSON.stringify(tags.slice(0, 50))}
  ...
  private buildUserPrompt(content: ParsedContent, userInstructions?: string): string {
      ...
      ${images.map((img, i) => `  Image ${i + 1}: ${img.alt || 'no alt text'} — ${img.url?.slice(0, 80) || 'no url'}`).join('\n') || '(none)'}

      Content (HTML):
      ${bodyHtml.slice(0, 8000)}
      ${bodyHtml.length > 8000 ? `... (truncated, full content is ${bodyHtml.length} chars)` : ''}`;
  ...
  private fallbackContent(content: ParsedContent): PublishableContent {
      ...
      return {
              ...
              status: 'draft',
              seo,
              featured_image: images.find((img) => img.is_featured) || images[0],
      };
  }
